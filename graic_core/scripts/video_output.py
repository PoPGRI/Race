...
class VideoOutput:
    def __init__(self, role_name='ego_vehicle'):
        ...
        self.videoPub = rospy.Publisher("/carla/%s/video_output" % role_name,
                                        Image,
                                        queue_size=None)
        self.videoSub = rospy.Subscriber(
            "/carla/%s/rgb_view/image" % role_name, Image, self.imageCallback)
        self.scoreSub = rospy.Subscriber("/carla/%s/score" % role_name,
                                         Float32, self.scoreCallback)
        self.reachedSub = rospy.Subscriber("/carla/%s/reached" % role_name,
                                           String, self.reachedCallback)
        self.collisionSub = rospy.Subscriber(
            "/carla/%s/collision_detail" % role_name, String,
    ...
    def imageCallback(self, data):
        ...
        outImg = cv2.putText(img, scoreStr, self.orgScore, self.font,
                             self.fontScale, self.color, self.thickness,
                             cv2.LINE_AA)
        outImg = cv2.putText(outImg, reachedStr, self.orgReached, self.font,
                             self.fontScale, self.color, self.thickness,
                             cv2.LINE_AA)
        outImg = cv2.putText(outImg, collisionStr, self.orgCollision,
